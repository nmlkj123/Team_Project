<%@page import="mylist.mylistlikePaging"%>
...
String pg =request.getParameter("pg");
 ...
 ResultSet rs3=stmt.executeQuery("select count(*) as total from mylist where userid='"+memId+"' and like_spot>0");
 rs3.next();
 int endNum=Integer.parseInt(pg)*6;
 int startNum=endNum-5;
 int totalA=rs3.getInt("total");
 mylistlikePaging touristPaging=new mylistlikePaging(); 
  ...
  ResultSet rs4=stmt.executeQuery("select count(*) as total from mylist where userid='"+memId+"' and jjim_spot>0");
  rs4.next();
  int totalB=rs4.getInt("total");
  ResultSet rs2 =stmt.executeQuery("select * from(select rownum rn ,t.* from(select * from mylist where userid='"+memId+"' and like_spot>0 )t) where rn>='"+startNum+"' and rn<='"+endNum+"'");
...
<!-- 마이페 -->
 <div class="container">
  <div class="content">
   <div class="profile_zone">
     ...
     <div class="list_zone">
      ...
      <dl>
       ...
       <dd><%=totalB %></dd> <!-- select count(*) cnt1 from tour_like_list
       				 where userid='"+sessionId+"' -->
      </dl>
      
      <dl>
       <dt><a href="myPage2.jsp?viewPage=1">나의 좋아요 여행지</a></dt> <!-- 링크연결 -->
       <dd><%=totalA %></dd> <!-- select count(*) cnt2 from tour_like_list
       				 where userid='"+sessionId+"' -->
      </dl>
   ...
   <!-- 목록출력 -->
   <div class="item_zone">
   <h3>&nbsp;나의 좋아요 여행지(<%=totalA %>)</h3><!-- cnt1 연결 -->
    <table class="items">
     <tr>
     <%
     int no =1;
     while(rs2.next()) {
    	 ...
    	 String unq = rs2.getString("like_spot");
