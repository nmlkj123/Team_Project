...
<%
	...
	int endNum=Integer.parseInt(pg)*5;
	   ...
	   ResultSet trs = stmt.executeQuery("select count(*)as total from shopping_review where pid='"+dpid+"'");
	   trs.next(); 
	   int totalA=trs.getInt("total");
	   ShoppingReviewPaging touristPaging=new ShoppingReviewPaging(); 
	...
	ResultSet rs=stmt.executeQuery("select * from(select rownum as rn ,t.* from(select unq,pid,id,name,content,image,hashtag,star,logtime from shopping_review where pid='"+dpid+"' order by unq desc)t) where rn>='"+startNum+"' and rn<='"+endNum+"'");
...
<!---->
	...
	<!---->
		...
		<%while(rs.next()){
			...
			ResultSet rsrc;
			rsrc=stmt2.executeQuery("select count(*) as cnt from shopping_review_board where pid="+unq+"");
			rsrc.next();
			String cnt=rsrc.getString("cnt");
			rsrc.close();
			
			rsrc=stmt2.executeQuery("select count(*) as lcnt from shopping_review_like where pid="+unq+"");
			rsrc.next();
		...
		<!---->
			...
			<%
				ResultSet rs2 =stmt3.executeQuery("select id,name,content,logtime,pid from shopping_review_board where pid="+unq+" order by logtime asc");
