...
String totalsql="select count(*) total from tourist_spot";
...
String   sql="select b.* from(select "; 
		...
		sql+=" from tourist_spot s , tourist t ";
...
<div class="container">
	...
	<table class="table1">
		...
		<tr>
			...
			<td><%=rn %></td>
			<td><%=dname %></td>
			<td><a href="tourDetail.jsp?unq=<%=unq%>"><%=name %></a></td>
			<td><%=addr %></td>
			<td><%=tel %></td>
  ...
  for(int i=1; i<=totalPage; i++){
	  %>
	  <a href="SpotList.jsp?viewPage=<%=i%>"><%=i %></a>
	  <%
